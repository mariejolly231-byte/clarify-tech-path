...
const PLANS: PlanItem[] = [
 ...
 " Toute donnée patient = C3 → aucun outil SaaS sans cadre RGPD",
 ],
 stack: "Claude en local (20$/mois) · NotebookLM (gratuit, données non sensibles) · Tally (gratuit, formulaires non médicaux)",
 idee: " Formulaire pré-consultation uniquement avec consentement explicite et hébergement EU obligatoire",
 },
 {
 id: "desousa",
 semaine: [
 ...
 " Toujours validation humaine avant envoi de tout document juridique",
 ],
 ...
 " Données de santé indirectes : accès strictement limité, hébergement EU",
 ],
...
→ Utilise le prompt copilote ci-dessous.`,
 ...
 Outils · {c.tools}
  ...
  {c.link && (
    c.link.href.startsWith("[") ? (
      <span className="inline-flex items-center gap-1.5 rounded-md border border-dashed border-primary/40 bg-background/60 px-2.5 py-1 text-[11px] text-muted-foreground">
        <span aria-hidden>🔗</span> Lien à ajouter — {c.link.label}
      </span>
    ) : (
      <a
        href={c.link.href}
 ...
 src={logoAsset.url}
 alt="Summit Flow"
 className="mx-auto h-16 w-16 rounded-full ring-1 ring-border"
 />

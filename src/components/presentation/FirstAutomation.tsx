...
const PLATFORMS = [
  { icon: "📱", name: "iPhone / iPad", tool: "App Raccourcis (native)" },
  { icon: "🤖", name: "Android", tool: "Automate ou Google Assistant Routines" },
  { icon: "🍎", name: "Mac", tool: "App Raccourcis (native depuis macOS Monterey)" },
  { icon: "🪟", name: "Windows", tool: "Power Automate Desktop (gratuit Microsoft)" },
];
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {PLATFORMS.map((p) => (
            <div key={p.name} className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-2 font-serif text-base text-foreground">{p.name}</div>

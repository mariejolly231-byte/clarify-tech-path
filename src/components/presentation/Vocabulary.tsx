...
export function Vocabulary() {
  return (
    <Section id="tri" num="06" eyebrow="Partie 2 · Comprendre les outils" title="Le lexique du randonneur">
      {/* BLOC 1 — Pause cerveau / IA */}
      <div className="mb-14 rounded-2xl border border-border bg-card p-7 shadow-sm animate-fade-in">
        <div className="mb-2 text-[11px] uppercase tracking-[0.2em] text-primary">Petite pause</div>
        ...
        <div className="mt-6 space-y-3 text-[15px] leading-relaxed text-foreground/85">
          <p>Quand on parle d'intelligence artificielle, on fait souvent comme si elle pensait « comme nous ».</p>
          <p>
            En réalité, le cerveau humain et l'IA peuvent parfois produire des résultats proches, mais ils n'utilisent
            pas les mêmes mécanismes.
          </p>
          <p>
            <span className="text-foreground">Le cerveau humain</span> est vivant, incarné, contextuel, émotionnel,
            plastique.
            <br />
            <span className="text-foreground">Une IA générative</span>, elle, calcule des probabilités à partir de très
            grandes quantités de données et produit une réponse plausible selon le contexte.
          </p>
        </div>

        <div className="my-8 flex items-center gap-4">
          <span className="h-px flex-1 bg-border" />
          <span className="font-serif text-2xl text-primary md:text-3xl">Comparer n'est pas confondre.</span>
          <span className="h-px flex-1 bg-border" />
        </div>
      ...
      <div className="mb-10">
        <div className="mb-2 text-[11px] uppercase tracking-[0.2em] text-primary">On remet de l'ordre</div>
        <h3 className="font-serif text-3xl text-foreground md:text-4xl">
          No code, IA, gloubi-boulga : <span className="text-primary">on distingue les rôles.</span>
        </h3>
        <p className="mt-2 text-base italic text-muted-foreground">Quand tout se mélange, on clarifie les rôles.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {CARDS.map((c) => (
          ...
          >
            ...
            <p className="mt-3 text-[15px] leading-relaxed text-foreground/90">« {c.def} »</p>
            <div className="mt-4 border-t border-foreground/10 pt-3">
              <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Exemple</div>
      ...
      <figure className="mt-10 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        ...
        <figcaption className="border-t border-border bg-stone-soft px-6 py-3 text-center text-xs italic text-muted-foreground">
          Assistant IA = répond &nbsp;·&nbsp; Automatisation = exécute &nbsp;·&nbsp; Agent IA = organise et agit
          davantage.
        </figcaption>
      </figure>
